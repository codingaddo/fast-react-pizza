    ...
    <div className=" px-4 py-6">
      ...
      <Form method="POST">
        ...
        <div className=" mb-5 flex  flex-col gap-2 sm:flex-row sm:items-center">
          ...
          {formErrors?.phone && <p className="text-red-700 text-xs bg-red-100 rounded-md p-2 mt-2">{formErrors.phone}</p>}
          </div>
        </div>
        ...
        <div>
          <Button type={'primary'} disabled={isSubmitting} >{isSubmitting? 'Placing order':`Order now ${formatCurrency(totalPrice)}`}</Button>
        </div>
      </Form>
    </div>
  );
